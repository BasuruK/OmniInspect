...
// later stories (M3.4–M3.6). The MCP SDK server itself is created lazily on the first ServeStdio call so dependency wiring can stay in cmd/.
...
// buildAndRegister constructs the SDK server and registers every tool that has been implemented so far. New tools are added here as later stories
// land (M3.5, M3.6). The startedAt timestamp is captured here so all uptime computations share the same origin.
