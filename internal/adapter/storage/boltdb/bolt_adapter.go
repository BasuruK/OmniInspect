...
// legacy "cfg:" prefix to the new "DBconfig:" prefix.
// The databaseId field in JSON is preserved as-is.
...
func (ba *BoltAdapter) GetBroadcastMode() (domain.BroadcastMode, error) {
	if ba.db == nil {
		return domain.BroadcastModeGlobal, fmt.Errorf("GetBroadcastMode: boltAdapter not initialized")
	}
	...
	if err != nil {
		return domain.BroadcastModeGlobal, fmt.Errorf("GetBroadcastMode: %w", err)
	}
	return mode, nil
}
