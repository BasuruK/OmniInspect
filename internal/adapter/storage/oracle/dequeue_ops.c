...
/**
 * ExecuteDequeuProc - Executes the dequeue PL/SQL procedure with a wait time
 * 
 ...
 */
static int ExecuteDequeueProc(dpiConn* conn, dpiContext* context, const char* subscriberName, uint32_t batchSize, int32_t waitTime, dpiVar* outPayloadVar, dpiVar* outRawVar, uint32_t* outCount) {
    dpiStmt* stmt = NULL;
    dpiVar* subVar = NULL;
    ...
    if (dpiStmt_execute(stmt, 0,0) != DPI_SUCCESS) {
        ...
        fprintf(stderr, "[C ERROR] Failed to execute statement %s\n", errorInfo.message);
        goto cleanup;
    }
    ...
    *outCount = (uint32_t)countData->value.asInt64;
    ...
    cleanup:
        ...
        // TODO: check if outPayloadVar and outRawVar need to be released here or by caller

    return result;
}

/**
 * DequeueManyAndExtract - Dequeues messages and extracts their content and IDs with a wait time
 * 
 * @param conn: Pointer to the dpiConn representing the Oracle connection
 * @param context: Pointer to the dpiContext for error handling
...
int DequeueManyAndExtract(dpiConn* conn, dpiContext* context, const char* subscriberName, uint32_t batchSize, int32_t waitTime, TraceMessage** outMessages, TraceId** outIds, uint32_t* actualCount) {
    ...
    *actualCount = 0;
    ...
    // Execute Blocking Dequeue Procedure in PLSQL
    if (ExecuteDequeueProc(conn, context, subscriberName, batchSize, waitTime, payloadVar, rawVar, actualCount) != DPI_SUCCESS) {
        fprintf(stderr, "[C ERROR] Failed to execute dequeue procedure\n");
        goto cleanup;
    }

    // If no messages dequeued, return success with zero count
    if (*actualCount == 0) {
        result = 0;
        goto cleanup;
    }
