...
CREATE OR REPLACE PACKAGE TXEVENTQ_PERMISSION_CHECK_API AS
    ...
    -- Check if schema has CREATE SEQUENCE privilege
    FUNCTION Has_Create_Sequence_Priv(p_schema IN VARCHAR2) RETURN BOOLEAN;
    
    -- Check if schema has CREATE PROCEDURE privilege
    FUNCTION Has_Create_Procedure_Priv(p_schema IN VARCHAR2) RETURN BOOLEAN;
    
    -- Check if schema has AQ_ADMINISTRATOR_ROLE
    FUNCTION Has_AQ_Admin_Role(p_schema IN VARCHAR2) RETURN BOOLEAN;
    
    -- Check if schema has AQ_USER_ROLE
    FUNCTION Has_AQ_User_Role(p_schema IN VARCHAR2) RETURN BOOLEAN;
    
    -- Check if schema has EXECUTE on DBMS_AQADM
    FUNCTION Has_DBMS_AQADM_Exec(p_schema IN VARCHAR2) RETURN BOOLEAN;
    
    -- Check if schema has EXECUTE on DBMS_AQ
    FUNCTION Has_DBMS_AQ_Exec(p_schema IN VARCHAR2) RETURN BOOLEAN;
...
CREATE OR REPLACE PACKAGE BODY TXEVENTQ_PERMISSION_CHECK_API AS
    ...
    BEGIN
        v_report := 'TXEVENTQ_PERMISSION_CHECK|SCHEMA=' || p_schema || '|';
